...
let subjectId = document.querySelector('#subject-id').textContent

const COURSES_PER_PAGE = 10


fetch(`/api/courses/by_subject/${subjectId}/`).then(response => response.json()).then(data => initializeCourses(data, 1))
fetch(`/api/subjects/${subjectId}/`).then(response => response.json()).then(data => {
...
function initializeCourses(data, page) {
    ...
    if (pages <= 3) {
        for (let i=1; i <= pages; i++) {
            ...
            paging.setAttribute('href', `/courses/by_subject/${subjectId}?page=${i}`)
