...
import factory
from ..models import AdditionalFile
from ..api.serializers import AdditionalFileSerializer
...
def test_create_additional_file(client, section, delete_test_files):
    ...
    response = client.post(reverse('api:additionalfile-list'), data)
...
def test_update_additional_file(client, section, additional_file, delete_test_files):
    data = factory.build(dict, FACTORY_CLASS=ft.AdditinalFileFactory)
    data['section'] = section.id
    response = client.put(reverse('api:additionalfile-detail', kwargs={'pk': additional_file.id}), data)
...
def test_partial_update_additional_file(client, section, additional_file):
    ...
    response = client.patch(reverse('api:additionalfile-detail', kwargs={'pk': additional_file.id}), data)
...
def test_retrieve_additional_file(client, additional_file):
    response = client.get(reverse('api:additionalfile-detail', kwargs={'pk': additional_file.id}))
...
def test_list_additional_file(client, disconnect_signals):
    ...
    response = client.get(reverse('api:additionalfile-list'))
...
def test_destroy_additional_file(client, additional_file):
    response = client.delete(reverse('api:additionalfile-detail', kwargs={'pk': additional_file.id}))
